    ...
    games.forEach(game => {
        ...
        card.innerHTML = `
            ...
            <div class="game-info">
                ...
                <div class="game-author">👤 ${escapeHtml(game.author)} | 🟢 ${game.players} игроков</div>
...
async function startGameSession(gameData, gameName) {
    ...
    if (gameData && gameData.blocks && Array.isArray(gameData.blocks)) {
        gameData.blocks.forEach(block => {
            ...
            const mesh = createMesh(block.shape || 'cube', scale, block.color, block.opacity || 1);
...
async function renderMyProjects() {
    ...
    try {
        ...
        myGames.forEach(game => {
            ...
            try {
                if (game.data) {
                    const parsed = typeof game.data === 'string' ? JSON.parse(game.data) : game.data;
                    gameDataSafe = parsed ? 'ok' : '';
                }
            } catch(e) { gameDataSafe = 'error'; }
            card.innerHTML = `
                <div class="game-title">${escapeHtml(game.name)}</div>
                <div class="game-desc" style="font-size:12px;color:#aaa;">${escapeHtml(game.description || 'Без описания')}</div>
                <div style="margin-top:12px;display:flex;gap:8px;">
                    <button class="play-btn-small" data-id="${game.id}" data-hasdata="${gameDataSafe}" style="background:#ff5722;border:none;padding:4px 12px;border-radius:20px;cursor:pointer;">Одиночная игра</button>
                    <button class="host-btn-small" data-id="${game.id}" data-hasdata="${gameDataSafe}" style="background:#4a6e8a;border:none;padding:4px 12px;border-radius:20px;cursor:pointer;">🌐 Создать сервер</button>
        ...
        if (container) container.innerHTML = '<p>Ошибка загрузки проектов</p>';
    }
}
...
async function handlePlayClick(e) {
    ...
    try {
        ...
        if (game && game.data) {
            let gameData;
            try {
                gameData = typeof game.data === 'string' ? JSON.parse(game.data) : game.data;
            } catch(parseErr) {
            ...
            if (gameData && gameData.blocks) {
                ...
                alert('Ошибка: данные игры повреждены');
            }
        } else {
...
function renderFriendsList() {
    ...
    (currentUser.friends || []).forEach(friend => {
        ...
        div.innerHTML = `<span>${escapeHtml(friend)}</span><div class="friend-actions"><button class="removeFriendBtn" data-friend="${escapeHtml(friend)}">❌</button></div>`;
        container.appendChild(div);
    });
...
async function renderChat() {
    ...
    try {
        ...
        messages.forEach(msg => { 
            const div = document.createElement('div'); 
            div.innerHTML = `<span style="color:#ffaa44;">[${escapeHtml(msg.time)}]</span> <b>${escapeHtml(msg.username)}:</b> ${escapeHtml(msg.text)}`; 
            container.appendChild(div); 
        });
...
async function renderReports() {
    ...
    try {
        ...
        reportsData.forEach(r => { 
            ...
            div.innerHTML = `<b>${escapeHtml(r.username)}</b> (${escapeHtml(r.time)}): ${escapeHtml(r.text)}`; 
            container.appendChild(div); 
        });
    } catch (e) {
...
async function updateUIafterAuth() {
    if (!currentUser) return;
    document.getElementById('usernameDisplay').innerText = escapeHtml(currentUser.username) + (currentUser.isGuest ? ' (гость)' : '');
